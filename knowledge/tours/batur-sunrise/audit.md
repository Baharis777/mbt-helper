...
- `knowledge/imports/batur-import-notes.md`
...
- Текст основной страницы в `knowledge/imports/batur-source-snapshot.md`: NEEDS_INPUT
- Точные названия вариантов в `knowledge/imports/batur-source-snapshot.md`: NEEDS_INPUT
- Сырой текст каждого якорного варианта в `knowledge/imports/batur-source-snapshot.md`: NEEDS_INPUT
...
1. Сначала вручную собрать текст страницы и якорных блоков в `knowledge/imports/batur-source-snapshot.md`.
2. Использовать `knowledge/imports/batur-import-notes.md` как инструкцию и чеклист для менеджера.
3. Подтвердить точные названия вариантов и заменить временные ID только после проверки.
4. Заполнить `variant-matrix.md` перед расширением текстовых карточек вариантов.
5. Не заполнять variant-факты напрямую из памяти.
6. Не переносить цену, включения, трансфер или отмену из одного варианта в другой.
7. Использовать `page.md` только для общих правил выбора и предупреждений.
8. Использовать `live-status/batur.md` как единственный файл для текущей безопасности и доступности.
9. После заполнения матрицы добавить тестовые менеджерские вопросы для каждого важного сценария: безопасность, цена, ребенок, горячие источники, без восхождения, трансфер.
